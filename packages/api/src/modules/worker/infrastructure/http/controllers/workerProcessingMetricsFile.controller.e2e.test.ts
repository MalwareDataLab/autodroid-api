...
describe("E2E: WorkerProcessingMetricsFileController", () => {
  beforeEach(context => {
  ...
  it("should generate a metrics file upload url", async context => {
    ...
    const response = await context.request
      .post(`/worker/processing/${processing.id}/metrics_file/generate_upload`)
  ...
  it("should fail generating a metrics upload for a processing not found", async context => {
    const { accessToken } = await createAuthorizedWorker();

    const response = await context.request
      .post(
        `/worker/processing/${faker.string.uuid()}/metrics_file/generate_upload`,
      )
      .set("Authorization", `Bearer ${accessToken}`)
      .send({
        filename: faker.system.fileName(),
    ...
    expect(response.body).toMatchObject({
      code: "@worker_generate_processing_metrics_upload_file_service/PROCESSING_NOT_FOUND",
    });
  });
  ...
  it("should capture an uploaded metrics file", async context => {
    ...
    const metricsFile = await fileFactory.create({
      ...
      public_url: "https://example.com/metrics.csv",
    ...
    const response = await context.request
      .post(`/worker/processing/${processing.id}/metrics_file/uploaded`)
      .set("Authorization", `Bearer ${accessToken}`)
      .send();

    expect(response.status).toBe(200);
    expect(response.body).toMatchObject({ id: metricsFile.id });
  });
