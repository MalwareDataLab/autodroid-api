...
const validateSorting = <T>({
  ...
  const duplicatedFields = value.reduce<string[]>(
    (acc, item: SortingFieldSchema<string[]>, index) => {
      if (
        value.findIndex(compareItem => compareItem.field === item.field) !==
          index &&
  ...
  const validFields = allowed.map(field => field.toString());
